...
<p class="description">
  Подборка полезных видео по разработке игр: геймдизайн, программирование,
  графика, продакшн, маркетинг, разборы провалов и чужих ошибок. Всё то, что
  обычно приходится собирать по сотням вкладок, рекомендациям с Discord и
  случайным комментариям под видео.
</p>
...
<p class="description">
  ...
  , там же можно найти дополнительные комментарии, мысли и заметки от автора подборки.
  Если хотите не пропускать обновления — подписывайтесь.
</p>
      ...
      <li>
        ...
        >
          ...
          <div class="video-content">
            ...
            <time datetime={card.source.datetime} title={card.absoluteDate}
              >{card.relativeTime}</time
            >
          </div>
        </a>
      </li>
  ...
  <div class="loader" bind:this={sentinel}>
    ...
    {:else if hasLoadError}
      <button
        class="retry-button"
        type="button"
        onclick={() => loadNextPage({ force: true })}>Повторить загрузку</button
      >
    {:else if !hasMore}
