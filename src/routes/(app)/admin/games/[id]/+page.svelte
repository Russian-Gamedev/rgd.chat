...
let game = $state<GameEditor | null>(null);
let published = $state<GameDetails | null>(null);
...
async function invalidateGames() {
	await Promise.all([
		...
		invalidate(`games:details:${game?.slug ?? page.params.id}`),
		invalidate(`games:details:${game?.id ?? page.params.id}`),
		invalidate('games:tags'),
		invalidate('games:mine'),
		game?.credits.owner_id ? invalidate(`users:${game.credits.owner_id}:games`) : Promise.resolve()
	]);
}
...
{:else if game}
	<div class="page-header"><div><h1>{game.title}</h1><p class="status">ID: {game.id} · Версия v{game.workflow.version} · Статус: {game.workflow.status}</p><div class="owner-line">Владелец: <UserIdentity id={game.credits.owner_id} /></div><p class="status">Изменено: {new Date(game.metadata.updated_at).toLocaleString('ru-RU')}</p>{#if game.workflow.submitted_at}<p class="status">Отправлено на ревью: {new Date(game.workflow.submitted_at).toLocaleString('ru-RU')}</p>{/if}</div>{#if game.workflow.has_published_version}<Button as="a" href={`/games/${game.slug}`} variant="outline">Публичная версия</Button>{/if}</div>
	<section><h2>Рабочая редакция</h2><GameRevisionPreview {game} /></section>
	{#if game.workflow.has_published_version}<section class="comparison-section"><h2>Сравнение с опубликованной версией v{game.workflow.published_version}</h2><GameRevisionComparison working={game} {published} /></section>{/if}
	<ReviewTimeline events={game.workflow.review_events} />
	<section class="decision-panel"><h2>Решение модератора</h2><p>Комментарий будет сохранён в истории ревью. Для возврата на доработку он обязателен.</p><textarea bind:value={comment} maxlength="2000" rows="6" placeholder="Комментарий модератора"></textarea><div class="decision-actions"><Button disabled={isMutating || game.workflow.status !== 'review'} onclick={() => (dialog = 'publish')}>Опубликовать</Button><Button variant="outline" color="error" disabled={isMutating || game.workflow.status !== 'review' || !validReviewComment(comment)} onclick={() => (dialog = 'changes')}>Вернуть на доработку</Button></div><div class="secondary-actions"><Button variant="ghost" disabled={isMutating} onclick={openOwner}>Сменить владельца</Button><Button variant="ghost" color="error" disabled={isMutating} onclick={() => (dialog = 'delete')}>Удалить игру</Button></div></section>

	<Modal open={dialog === 'publish'} title="Опубликовать редакцию" onClose={() => (dialog = null)}><p>Версия v{game.workflow.version} станет публичной.</p><div class="modal-actions"><Button disabled={isMutating} onclick={() => mutate('publish')}>Подтвердить публикацию</Button></div></Modal>
